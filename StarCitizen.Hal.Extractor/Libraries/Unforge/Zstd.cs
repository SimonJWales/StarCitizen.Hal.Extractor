...
namespace Zstd.Net
{
    ...
    public class InputStream : Stream
    {
        ...
        readonly IntPtr _zst;

        readonly byte[] _inputBufferArray;

        bool _closed;

        int _inputArrayPosition;
        ...
        readonly Zstd.Buffer _inputBuffer = new Zstd.Buffer();

        readonly Zstd.Buffer _outputBuffer = new Zstd.Buffer();
    ...
    internal static class Zstd
    {
        ...
        static Zstd()
        {
            if (Environment.Is64BitProcess) Library = new Zstd_x64 { };

            else Library = new Zstd_x86 { };
        }

        internal static void CheckError(UIntPtr x)
        {
            var code = Library.IsError(x);
        ...
        class Zstd_x64 : IImportZstd
        {
            ...
            public int IsError(UIntPtr code) 
            { 
                return zstd_isError(code); 
            }

            public string GetErrorName(UIntPtr code) 
            { 
                return zstd_getErrorName(code); 
            }

            public IntPtr CreateCStream() 
            { 
                return zstd_createCStream(); 
            }

            public UIntPtr FreeCStream(IntPtr zcs) 
            { 
                return zstd_freeCStream(zcs); 
            }

            public UIntPtr InitCStream(IntPtr zcs, int compressionLevel) 
            { 
                return zstd_initCStream(zcs, compressionLevel); 
            }

            public UIntPtr CompressStream(
                IntPtr zcs, 
                Buffer outputBuffer, 
                Buffer inputBuffer) 
            { 
                return zstd_compressStream(zcs, outputBuffer, inputBuffer); 
            }

            public UIntPtr FlushStream(IntPtr zcs, Buffer outputBuffer) 
            { 
                return zstd_flushStream(zcs, outputBuffer); 
            }

            public UIntPtr EndStream(IntPtr zcs, Buffer outputBuffer) 
            { 
                return zstd_endStream(zcs, outputBuffer); 
            }

            public UIntPtr CStreamInSize() 
            { 
                return zstd_CStreamInSize(); 
            }

            public UIntPtr CStreamOutSize() 
            { 
                return zstd_CStreamOutSize(); 
            }

            public IntPtr CreateDStream() 
            { 
                return zstd_createDStream(); 
            }

            public UIntPtr FreeDStream(IntPtr zcs) 
            { 
                return zstd_freeDStream(zcs); 
            }

            public UIntPtr InitDStream(IntPtr zcs) 
            { 
                return zstd_initDStream(zcs); 
            }

            public UIntPtr DecompressStream(
                IntPtr zcs, 
                Buffer outputBuffer, 
                Buffer inputBuffer) 
            { 
                return zstd_decompressStream(zcs, outputBuffer, inputBuffer); 
            }

            public UIntPtr DStreamInSize() 
            { 
                return zstd_DStreamInSize(); 
            }

            public UIntPtr DStreamOutSize() 
            { 
        ...
        class Zstd_x86 : IImportZstd
        {
            #region DllImports

            const string DllName = @"x86\libzstd";
