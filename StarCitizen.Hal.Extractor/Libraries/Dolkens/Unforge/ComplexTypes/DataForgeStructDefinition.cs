using StarCitizen.Hal.Extractor.Library.Dolkens.Unforge.Enums;
using System;
...
namespace StarCitizen.Hal.Extractor.Library.Dolkens.Unforge.ComplexTypes
{
    public class DataForgeStructDefinition : DataForgeSerializable
    {
        ...
        public XmlElement Read(string? name = null)
        {
            ...
            while (baseStruct.ParentTypeIndex != 0xFFFFFFFF)
            {
                ...
                properties.InsertRange(0,
                    from index in Enumerable.Range(
                        baseStruct.FirstAttributeIndex, 
                        baseStruct.AttributeCount)
                    let property = DocumentRoot.PropertyDefinitionTable[index]
                    // where !properties.Contains(property)
                    select property);
            }

            var element = DocumentRoot.CreateElement(name ?? baseStruct.Name);
            ...
            foreach (var node in properties)
            {
                ...
                if (node.ConversionType == EConversionType.varAttribute)
                {
                    ...
                    {
                        var dataStruct = DocumentRoot.StructDefinitionTable[node.StructIndex];
                    ...
                    {
                        var parentSP = DocumentRoot.CreateElement(node.Name);

                        var emptySP = DocumentRoot.CreateElement(
                            string.Format(
                                "{0}", 
                                node.DataType));
                        ...
                        DocumentRoot.Require_ClassMapping.Add(
                            new ClassMapping { 
                                Node = emptySP, 
                                StructIndex = (ushort)_br.ReadUInt32(), 
                                RecordIndex = (int)_br.ReadUInt32() 
                            });
                    }
                    else
                    {
                ...
                {
                    var arrayCount = _br.ReadUInt32();

                    var firstIndex = _br.ReadUInt32();

                    var child = DocumentRoot.CreateElement(node.Name);
                    ...
                    for (var i = 0; i < arrayCount; i++)
                    {
                        switch (node.DataType)
                        {
                            ...
                            case EDataType.varClass:
                                var emptyC = DocumentRoot.CreateElement(string.Format("{0}", node.DataType));
                            ...
                            case EDataType.varStrongPointer:
                                var emptySP = DocumentRoot.CreateElement(string.Format("{0}", node.DataType));

                                child.AppendChild(emptySP);

                                DocumentRoot.Require_StrongMapping.Add(
                                    new ClassMapping { 
                                        Node = emptySP, 
                                        StructIndex = node.StructIndex, 
                                        RecordIndex = (int)(firstIndex + i) 
                            ...
                            case EDataType.varWeakPointer:
                                var weakPointerElement = DocumentRoot.CreateElement("WeakPointer");

                                var weakPointerAttribute = DocumentRoot.CreateAttribute(node.Name);

                                weakPointerElement.Attributes.Append(weakPointerAttribute);

                                child.AppendChild(weakPointerElement);

                                DocumentRoot.Require_WeakMapping1.Add(
                                    new ClassMapping { 
        ...
        public string Export(string assemblyName = "HoloXPLOR.Data.DataForge")
        {
            ...
            if (ParentTypeIndex != 0xFFFFFFFF)
            {
                sb.AppendFormat(" : {0}", DocumentRoot.StructDefinitionTable[ParentTypeIndex].Name);
            }
            sb.AppendLine();
            sb.AppendLine(@"    {");
            ...
            {
                var property = DocumentRoot.PropertyDefinitionTable[i];
                ...
                switch (property.DataType)
                {
                    ...
                    case EDataType.varReference:
                        ...
                        {
                            sb.AppendFormat("        public Guid{2} {1} {{ get; set; }}", DocumentRoot.StructDefinitionTable[property.StructIndex].Name, propertyName, arraySuffix);
                        }
                        break;
                    case EDataType.varLocale:
                    case EDataType.varWeakPointer:
                        ...
                        {
                            sb.AppendFormat("        public String{2} {1} {{ get; set; }}", DocumentRoot.StructDefinitionTable[property.StructIndex].Name, propertyName, arraySuffix);
                        }
                        break;
                    default:
                        sb.AppendFormat("        public {3}{0}{2} {1} {{ get; set; }}", property.DataType.ToString().Replace("var", ""), propertyName, arraySuffix, arrayPrefix);
