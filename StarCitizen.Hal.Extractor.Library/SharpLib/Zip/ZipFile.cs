...
namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    public class KeysRequiredEventArgs(
            string name,
            byte[] keyValue) : EventArgs
    {
        ...
        /// </summary>
        public byte[]? Key { get; set; } = keyValue;
        #endregion
    ...
    public class ZipFile : IEnumerable, IDisposable
    {
        ...
        public IEnumerator GetEnumerator()
        {
            return isDisposed_ ?
                ...
                (IEnumerator)new ZipEntryEnumerator(entries_);
        }
        ...
        public Stream GetInputStream(ZipEntry entry)
        {
            ...
            if ((index < 0) ||
                ...
                (entries_[index].Name != entry.Name))
            {
        ...
        public Stream GetInputStream(long entryIndex)
        {
            ...
            Stream result = new PartialInputStream(
                this,
                start,
                entries_[entryIndex].CompressedSize);

            if (entries_[entryIndex].IsAesCrypted == true)
            {
                result = CreateAndInitAesDecryptionStream(result);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (entries_.Length == 0)
                    {
                        ...
                        using ZipHelperStream zhs = new(baseStream_);
		...
		*/
            ...
            bool updateCrc)
        {
            ...
            // Initializing the CRC32 object only if required
            Crc32? crc = updateCrc ? new Crc32() : null;
            ...
            while (bytesToCopy > 0)
            {
                int readSize = (int)Math.Min(buffer.Length, bytesToCopy);
            ...
            if (totalBytesRead != bytesToCopy)
            {
                throw new ZipException($"Failed to copy bytes: expected {bytesToCopy}, read {totalBytesRead}");
            }

            // Update CRC value after all bytes are processed
            if (updateCrc && crc is not null)
            {
        ...
        {
            Stream? streamResult = baseStream_;

            if (entry.IsCrypted == true)
            {
                streamResult = CreateAndInitEncryptionStream(
                    streamResult, 
                    entry);
            }
            ...
            {
                throw new ZipException("Unable to obtain output stream");
            }

            switch (entry.CompressionMethod)
            {
                case CompressionMethod.Stored:
                    streamResult = new UncompressedStream(streamResult);
            ...
            ZipUpdate update)
        {
            ...
            if (update.Entry.IsFile)
            {
                ...
                source ??= updateDataSource_.GetSource(
                    update.Entry,
                    update.Filename);
            }
            ...
            if (source != null)
            {
                using (source)
                {
                    ...
                    {
                        await CopyBytesAsync(update, output, source, sourceStreamLength, true);
                    }
        ...
        class UpdateComparer : IComparer<ZipUpdate>
        {
            ...
            /// <returns>Compare cryptoStreamResult.</returns>
                    ...
                    if (update != null)
                    {
                        switch (update.Command)
                        {
                            ...
                            case UpdateCommand.Add:
                                ...
                                await AddEntry(workFile, update);
        ...
        long LocateBlockWithSignature(
            int signature,
            long endLocation,
            int minimumBlockSize,
            int maximumVariableData)
        {
            using ZipHelperStream les = new(baseStream_);

            return les.LocateBlockWithSignature(
                signature,
                endLocation,
                minimumBlockSize,
                maximumVariableData);
        ...
        void ReadEntries()
        {
                ...
                (offsetOfCentralDir == 0xffffffff))
            {
                ...
                long offset = LocateBlockWithSignature(
            ...
            if (!isZip64 &&
                (offsetOfCentralDir < locatedEndOfCentralDir - (4 + (long)centralDirSize)))
            {
            ...
            baseStream_.Seek(
                offsetOfFirstEntry + offsetOfCentralDir,
                SeekOrigin.Begin);

            for (ulong i = 0; i < entriesForThisDisk; i++)
            {
                var headerOffset = baseStream_.Position;
                ...
                string name = ZipConstants.ConvertToStringExt(
                    bitFlags,
                    buffer,
                    nameLen);
        ...
        //Stream CreateAndInitAesDecryptionStream(Stream baseStream)
        //{
        ...
        Stream CreateAndInitAesDecryptionStream(Stream baseStream)
        {
            // Use Aes.Create() to automatically select the best implementation
            using Aes aes = Aes.Create();

            aes.Key = key;  // Ensure that 'key' is defined and available in your scope

            aes.IV = new byte[16];  // Sets an all-zero initialization vector (IV)
            ...
            // CryptoStream for decryption
            CryptoStream cryptoStream = new CryptoStream(
                baseStream,
                decryptor,
                CryptoStreamMode.Read);

            // MemoryStream for storing decrypted data
            MemoryStream memoryStream = new MemoryStream();

            // Copy decrypted data to the memory stream
            cryptoStream.CopyTo(memoryStream);

            // Optionally trim NULL bytes from the end of the decrypted content
            memoryStream.Seek(-1, SeekOrigin.End);
            while (memoryStream.Position > 0 && memoryStream.ReadByte() == 0)
                memoryStream.Seek(-2, SeekOrigin.Current);

            memoryStream.SetLength(memoryStream.Position + 1);
            ...
            ZipEntry entry)
        {
                ...
                || (entry.Flags & (int)GeneralBitFlags.StrongEncryption) == 0)
            {
                ...
                result = new CryptoStream(
                    baseStream,
                    classicManaged.CreateDecryptor(
                        key,
                        null),
                    CryptoStreamMode.Read);
        ...
        Stream? CreateAndInitEncryptionStream(
            Stream baseStream,
            ZipEntry entry)
        {
            CryptoStream? cryptoStreamResult = null;
                ...
                || (entry.Flags & (int)GeneralBitFlags.StrongEncryption) == 0)
            {
                ...
                cryptoStreamResult = new CryptoStream(new UncompressedStream(baseStream),
            ...
            return cryptoStreamResult;
        }

        static void CheckClassicPassword(
        ...
        static void WriteEncryptionHeader(
            Stream stream,
            long crcValue)
        {
            byte[] cryptBuffer = new byte[ZipConstants.CryptoHeaderSize];
        ...
        /// </summary>
        class UncompressedStream(Stream baseStream) : Stream
        {
    ...
    public class DynamicDiskDataSource : IDynamicDataSource
    {
            ...
            string name)
        {
            ...
            //Stream cryptoStreamResult = null;

            //if (name != null) {
            //	cryptoStreamResult = File.Open(name, FileMode.Open, FileAccess.Read, FileShare.Read);
            //}
